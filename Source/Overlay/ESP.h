...
public:
	static void DrawFPS(ImGuiIO& io)
	{
		...
		if (GameData.Config.ESP.DataSwitch)
		{
			...
			// 渲染其他信息
			RenderHelper::StrokeText(Utils::StringToUTF8(std::format("{} {}\nF2: Aimbot Toggle: {}\nCombat Mode: {}\nGrouped Display: {}  \nAimbot Config: Preset {}\nCurrent Damage: {} \nCurrent Rank: {} \nBlind Spot Count: {}",
				Scene,
				(int)io.Framerate,
				GameData.Config.AimBot.Enable ? "On" : "Of",
				GameData.Config.ESP.FocusMode ? "On" : "Of",
				Items,
			...
			RenderHelper::StrokeText(Utils::StringToUTF8(std::format("Spectator: {}",
				spectatedCount
	...
	static void DrawLocalPlayerProject()
	{
		if (GameData.Config.Project.ShowChareTime)
		{
			...
			if (GameData.LocalPlayerInfo.ElapsedCookingTime > 0 && (int)GameData.LocalPlayerInfo.WeaponClassByte == 10)
			{
				...
				if (GameData.LocalPlayerInfo.WeaponName == "Flashbang")
				{
					totalTime /= 2.f;
				}
	...
	static void DrawVehicles(std::unordered_map<uint64_t, VehicleInfo> Vehicles)
	{
		...
		for (auto& Item : Vectors)
		{
			...
			if (GameData.Config.Vehicle.Health)
			{
				...
				//显示血量数值
				char healthText[16];
				snprintf(healthText, sizeof(healthText), U8("HP:%.0f%"), HealthPercent * 100.0f);//格式化“50%”
			...
			if (GameData.Config.Vehicle.Durability)
			{
				...
				//显示血量数值
				char fuelText[16];
				snprintf(fuelText, sizeof(fuelText), U8("HP:%.0f%"), FuelPercent * 100.0f);//格式化“50%”
	...
	static void DrawPlayers(std::unordered_map<uint64_t, Player> Players)
	{
		...
		for (auto& Item : PlayersVector)
		{
			...
			if (GameData.Config.ESP.Partner && Player.PartnerLevel > 0 && bShowInfo) {
				...
				InfoText += std::format("SidekickLvl.{}", (int)Player.PartnerLevel);
		...
		if (GameData.Config.ESP.DangerWarning)
		{
			...
			if (M200PlayerCount > 0) {
				RenderHelper::StrokeText(Utils::StringToUTF8(std::format(
					"Enemies within 200m: {}  Nearest enemy: {}m  Closest player's name: {}",
					M200PlayerCount,
					PalyerBB,
					DisNane
	...
	static void DrawPackages(std::unordered_map<uint64_t, PackageInfo> Packages)
	{
		...
		for (auto& Item : VectorItems)
		{
			...
			if (Package.Type == EntityType::AirDrop)
			{
				...
				std::string Text = Utils::StringToUTF8(std::format("Airdrop [{}M]", (int)Package.Distance));
				...
				std::string Text = Utils::StringToUTF8(std::format("DeathBox [{}M]", (int)Package.Distance));
